...
// One definition of what counts as a single measure, shared with the email.
// Two copies disagreed once and a twelve-period record was shown as six.
import { metricKey, displayLabel } from "../src/metrics.js";
...
function coverageOf(scoredPairs) {
  ...
  const metrics = Object.values(byMetric).map((m) => {
    ...
    const label = displayLabel(labels);
    return {
      metric: label,
      alsoCalled: labels.filter((l) => l !== label),
